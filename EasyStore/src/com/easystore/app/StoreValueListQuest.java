package com.easystore.app;

import java.util.ArrayList;

public class StoreValueListQuest extends ArrayList<StoreValueListQuest> {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 3526910288462753667L;
	String name = "";
	String value ="";
	/**
	 * @return the value
	 */
	public String getValue() {
		return value;
	}
	/**
	 * @param value the value to set
	 */
	public void setValue(String value) {
		this.value = value;
	}
	String type = "";
	String question = "";
	String atr1 = "";
	String atr2 = "";
	String atr3 = "";
	String index = "";
	String count="";
	String username = "";
	String password = "";
	String header="";
	String questionnaire_id = "";
	String patient_id = "";
	String answer0 = "",answer1 = "", answer2 = "",answer3 = "",answer4 = "",answer5 = "",answer6 = "",answer7 = "",answer8 = "",answer9 = "",answer10 = "",
		   answer11 = "",answer12 = "",answer13 = "",answer14 = "",answer15 = "",answer16 = "",answer17 = "",answer18 = "",answer19 = "",answer20 = "",
		   answer21 = "",answer22 = "",answer23 = "",answer24 = "",answer25 = "",answer26 = "",answer27 = "",answer28 = "",answer29 = "",answer30 = "",
		   answer31 = "",answer32 = "",answer33 = "",answer34 = "",answer35 = "",answer36 = "",answer37 = "",answer38 = "",answer39 = "",answer40 = "";
	String date = "";
	String temp = "";
	String device_id = "";
	
	/**
	 * @return the device_id
	 */
	public String getDevice_id() {
		return device_id;
	}
	/**
	 * @param device_id the device_id to set
	 */
	public void setDevice_id(String device_id) {
		this.device_id = device_id;
	}
	/**
	 * @return the temp
	 */
	public String getTemp() {
		return temp;
	}
	/**
	 * @param temp the temp to set
	 */
	public void setTemp(String temp) {
		this.temp = temp;
	}
	/**
	 * @return the questionnaire_id
	 */
	public String getQuestionnaire_id() {
		return questionnaire_id;
	}
	/**
	 * @param questionnaire_id the questionnaire_id to set
	 */
	public void setQuestionnaire_id(String questionnaire_id) {
		this.questionnaire_id = questionnaire_id;
	}
	/**
	 * @return the patient_id
	 */
	public String getPatient_id() {
		return patient_id;
	}
	/**
	 * @param patient_id the patient_id to set
	 */
	public void setPatient_id(String patient_id) {
		this.patient_id = patient_id;
	}
	/**
	 * @return the answer1
	 */
	public String getAnswer1() {
		return answer1;
	}
	/**
	 * @param answer1 the answer1 to set
	 */
	public void setAnswer1(String answer1) {
		this.answer1 = answer1;
	}
	/**
	 * @return the answer2
	 */
	public String getAnswer2() {
		return answer2;
	}
	/**
	 * @param answer2 the answer2 to set
	 */
	public void setAnswer2(String answer2) {
		this.answer2 = answer2;
	}
	/**
	 * @return the answer3
	 */
	public String getAnswer3() {
		return answer3;
	}
	/**
	 * @param answer3 the answer3 to set
	 */
	public void setAnswer3(String answer3) {
		this.answer3 = answer3;
	}
	/**
	 * @return the answer4
	 */
	public String getAnswer4() {
		return answer4;
	}
	/**
	 * @param answer4 the answer4 to set
	 */
	public void setAnswer4(String answer4) {
		this.answer4 = answer4;
	}
	/**
	 * @return the answer5
	 */
	public String getAnswer5() {
		return answer5;
	}
	/**
	 * @param answer5 the answer5 to set
	 */
	public void setAnswer5(String answer5) {
		this.answer5 = answer5;
	}
	/**
	 * @return the answer6
	 */
	public String getAnswer6() {
		return answer6;
	}
	/**
	 * @param answer6 the answer6 to set
	 */
	public void setAnswer6(String answer6) {
		this.answer6 = answer6;
	}
	/**
	 * @return the answer7
	 */
	public String getAnswer7() {
		return answer7;
	}
	/**
	 * @param answer7 the answer7 to set
	 */
	public void setAnswer7(String answer7) {
		this.answer7 = answer7;
	}
	/**
	 * @return the answer8
	 */
	public String getAnswer8() {
		return answer8;
	}
	/**
	 * @param answer8 the answer8 to set
	 */
	public void setAnswer8(String answer8) {
		this.answer8 = answer8;
	}
	/**
	 * @return the answer9
	 */
	public String getAnswer9() {
		return answer9;
	}
	/**
	 * @param answer9 the answer9 to set
	 */
	public void setAnswer9(String answer9) {
		this.answer9 = answer9;
	}
	/**
	 * @return the answer10
	 */
	public String getAnswer10() {
		return answer10;
	}
	/**
	 * @param answer10 the answer10 to set
	 */
	public void setAnswer10(String answer10) {
		this.answer10 = answer10;
	}
	/**
	 * @return the answer11
	 */
	public String getAnswer11() {
		return answer11;
	}
	/**
	 * @param answer11 the answer11 to set
	 */
	public void setAnswer11(String answer11) {
		this.answer11 = answer11;
	}
	/**
	 * @return the answer12
	 */
	public String getAnswer12() {
		return answer12;
	}
	/**
	 * @param answer12 the answer12 to set
	 */
	public void setAnswer12(String answer12) {
		this.answer12 = answer12;
	}
	/**
	 * @return the answer13
	 */
	public String getAnswer13() {
		return answer13;
	}
	/**
	 * @param answer13 the answer13 to set
	 */
	public void setAnswer13(String answer13) {
		this.answer13 = answer13;
	}
	/**
	 * @return the answer14
	 */
	public String getAnswer14() {
		return answer14;
	}
	/**
	 * @param answer14 the answer14 to set
	 */
	public void setAnswer14(String answer14) {
		this.answer14 = answer14;
	}
	/**
	 * @return the answer15
	 */
	public String getAnswer15() {
		return answer15;
	}
	/**
	 * @param answer15 the answer15 to set
	 */
	public void setAnswer15(String answer15) {
		this.answer15 = answer15;
	}
	/**
	 * @return the answer16
	 */
	public String getAnswer16() {
		return answer16;
	}
	/**
	 * @param answer16 the answer16 to set
	 */
	public void setAnswer16(String answer16) {
		this.answer16 = answer16;
	}
	/**
	 * @return the answer17
	 */
	public String getAnswer17() {
		return answer17;
	}
	/**
	 * @param answer17 the answer17 to set
	 */
	public void setAnswer17(String answer17) {
		this.answer17 = answer17;
	}
	/**
	 * @return the answer18
	 */
	public String getAnswer18() {
		return answer18;
	}
	/**
	 * @param answer18 the answer18 to set
	 */
	public void setAnswer18(String answer18) {
		this.answer18 = answer18;
	}
	/**
	 * @return the answer19
	 */
	public String getAnswer19() {
		return answer19;
	}
	/**
	 * @param answer19 the answer19 to set
	 */
	public void setAnswer19(String answer19) {
		this.answer19 = answer19;
	}
	/**
	 * @return the answer20
	 */
	public String getAnswer20() {
		return answer20;
	}
	/**
	 * @param answer20 the answer20 to set
	 */
	public void setAnswer20(String answer20) {
		this.answer20 = answer20;
	}
	/**
	 * @return the answer21
	 */
	public String getAnswer21() {
		return answer21;
	}
	/**
	 * @param answer21 the answer21 to set
	 */
	public void setAnswer21(String answer21) {
		this.answer21 = answer21;
	}
	/**
	 * @return the answer22
	 */
	public String getAnswer22() {
		return answer22;
	}
	/**
	 * @param answer22 the answer22 to set
	 */
	public void setAnswer22(String answer22) {
		this.answer22 = answer22;
	}
	/**
	 * @return the answer23
	 */
	public String getAnswer23() {
		return answer23;
	}
	/**
	 * @param answer23 the answer23 to set
	 */
	public void setAnswer23(String answer23) {
		this.answer23 = answer23;
	}
	/**
	 * @return the answer24
	 */
	public String getAnswer24() {
		return answer24;
	}
	/**
	 * @param answer24 the answer24 to set
	 */
	public void setAnswer24(String answer24) {
		this.answer24 = answer24;
	}
	/**
	 * @return the answer25
	 */
	public String getAnswer25() {
		return answer25;
	}
	/**
	 * @param answer25 the answer25 to set
	 */
	public void setAnswer25(String answer25) {
		this.answer25 = answer25;
	}
	/**
	 * @return the answer26
	 */
	public String getAnswer26() {
		return answer26;
	}
	/**
	 * @param answer26 the answer26 to set
	 */
	public void setAnswer26(String answer26) {
		this.answer26 = answer26;
	}
	/**
	 * @return the answer27
	 */
	public String getAnswer27() {
		return answer27;
	}
	/**
	 * @param answer27 the answer27 to set
	 */
	public void setAnswer27(String answer27) {
		this.answer27 = answer27;
	}
	/**
	 * @return the answer28
	 */
	public String getAnswer28() {
		return answer28;
	}
	/**
	 * @param answer28 the answer28 to set
	 */
	public void setAnswer28(String answer28) {
		this.answer28 = answer28;
	}
	/**
	 * @return the answer29
	 */
	public String getAnswer29() {
		return answer29;
	}
	/**
	 * @param answer29 the answer29 to set
	 */
	public void setAnswer29(String answer29) {
		this.answer29 = answer29;
	}
	/**
	 * @return the answer30
	 */
	public String getAnswer30() {
		return answer30;
	}
	/**
	 * @param answer30 the answer30 to set
	 */
	public void setAnswer30(String answer30) {
		this.answer30 = answer30;
	}
	/**
	 * @return the answer31
	 */
	public String getAnswer31() {
		return answer31;
	}
	/**
	 * @param answer31 the answer31 to set
	 */
	public void setAnswer31(String answer31) {
		this.answer31 = answer31;
	}
	/**
	 * @return the answer32
	 */
	public String getAnswer32() {
		return answer32;
	}
	/**
	 * @param answer32 the answer32 to set
	 */
	public void setAnswer32(String answer32) {
		this.answer32 = answer32;
	}
	/**
	 * @return the answer33
	 */
	public String getAnswer33() {
		return answer33;
	}
	/**
	 * @param answer33 the answer33 to set
	 */
	public void setAnswer33(String answer33) {
		this.answer33 = answer33;
	}
	/**
	 * @return the answer34
	 */
	public String getAnswer34() {
		return answer34;
	}
	/**
	 * @param answer34 the answer34 to set
	 */
	public void setAnswer34(String answer34) {
		this.answer34 = answer34;
	}
	/**
	 * @return the answer35
	 */
	public String getAnswer35() {
		return answer35;
	}
	/**
	 * @param answer35 the answer35 to set
	 */
	public void setAnswer35(String answer35) {
		this.answer35 = answer35;
	}
	/**
	 * @return the answer36
	 */
	public String getAnswer36() {
		return answer36;
	}
	/**
	 * @param answer36 the answer36 to set
	 */
	public void setAnswer36(String answer36) {
		this.answer36 = answer36;
	}
	/**
	 * @return the answer37
	 */
	public String getAnswer37() {
		return answer37;
	}
	/**
	 * @param answer37 the answer37 to set
	 */
	public void setAnswer37(String answer37) {
		this.answer37 = answer37;
	}
	/**
	 * @return the answer38
	 */
	public String getAnswer38() {
		return answer38;
	}
	/**
	 * @param answer38 the answer38 to set
	 */
	public void setAnswer38(String answer38) {
		this.answer38 = answer38;
	}
	/**
	 * @return the answer39
	 */
	public String getAnswer39() {
		return answer39;
	}
	/**
	 * @param answer39 the answer39 to set
	 */
	public void setAnswer39(String answer39) {
		this.answer39 = answer39;
	}
	/**
	 * @return the answer40
	 */
	public String getAnswer40() {
		return answer40;
	}
	/**
	 * @param answer40 the answer40 to set
	 */
	public void setAnswer40(String answer40) {
		this.answer40 = answer40;
	}
	/**
	 * @return the date
	 */
	public String getDate() {
		return date;
	}
	/**
	 * @param date the date to set
	 */
	public void setDate(String date) {
		this.date = date;
	}
	/**
	 * @return the header
	 */
	public String getHeader() {
		return header;
	}
	/**
	 * @param header the header to set
	 */
	public void setHeader(String header) {
		this.header = header;
	}
	/**
	 * @return the username
	 */
	public String getUsername() {
		return username;
	}
	/**
	 * @param username the username to set
	 */
	public void setUsername(String username) {
		this.username = username;
	}
	/**
	 * @return the password
	 */
	public String getPassword() {
		return password;
	}
	/**
	 * @param password the password to set
	 */
	public void setPassword(String password) {
		this.password = password;
	}
	/**
	 * @return the count
	 */
	public String getCount() {
		return count;
	}
	/**
	 * @param count the count to set
	 */
	public void setCount(String count) {
		this.count = count;
	}
	/**
	 * @return the index
	 */
	public String getIndex() {
		return index;
	}
	/**
	 * @param index the index to set
	 */
	public void setIndex(String index) {
		this.index = index;
	}
	/**
	 * @return the question
	 */
	public String getQuestion() {
		return question;
	}
	/**
	 * @param question the question to set
	 */
	public void setQuestion(String question) {
		this.question = question;
	}
	/**
	 * @return the atr1
	 */
	public String getAtr1() {
		return atr1;
	}
	/**
	 * @param atr1 the atr1 to set
	 */
	public void setAtr1(String atr1) {
		this.atr1 = atr1;
	}
	/**
	 * @return the atr2
	 */
	public String getAtr2() {
		return atr2;
	}
	/**
	 * @param atr2 the atr2 to set
	 */
	public void setAtr2(String atr2) {
		this.atr2 = atr2;
	}
	/**
	 * @return the atr3
	 */
	public String getAtr3() {
		return atr3;
	}
	/**
	 * @param atr3 the atr3 to set
	 */
	public void setAtr3(String atr3) {
		this.atr3 = atr3;
	}
	/**
	 * @return the name
	 */
	public String getName() {
		return name;
	}
	/**
	 * @param name the name to set
	 */
	public void setName(String name) {
		this.name = name;
	}
	/**
	 * @return the type
	 */
	public String getType() {
		return type;
	}
	/**
	 * @param type the type to set
	 */
	public void setType(String type) {
		this.type = type;
	}
	
	public void setAnswerj(int index, String str){
		switch (index) {
		case 0:
			setAnswer0(str);
			break;
		case 1:
			setAnswer1(str);
			break;
		case 2:
			setAnswer2(str);
			break;
		case 3:
			setAnswer3(str);
			break;
		case 4:
			setAnswer4(str);
			break;
		case 5:
			setAnswer5(str);
			break;
		case 6:
			setAnswer6(str);
			break;
		case 7:
			setAnswer7(str);
			break;
		case 8:
			setAnswer8(str);
			break;
		case 9:
			setAnswer9(str);
			break;
		case 10:
			setAnswer10(str);
			break;
		case 11:
			setAnswer11(str);
			break;
		case 12:
			setAnswer12(str);
			break;
		case 13:
			setAnswer13(str);
			break;
		case 14:
			setAnswer14(str);
			break;
		case 15:
			setAnswer15(str);
			break;
		case 16:
			setAnswer16(str);
			break;
		case 17:
			setAnswer17(str);
			break;
		case 18:
			setAnswer18(str);
			break;
		case 19:
			setAnswer19(str);
			break;
		case 20:
			setAnswer20(str);
			break;
		case 21:
			setAnswer21(str);
			break;
		case 22:
			setAnswer22(str);
			break;
		case 23:
			setAnswer23(str);
			break;
		case 24:
			setAnswer24(str);
			break;
		case 25:
			setAnswer25(str);
			break;
		case 26:
			setAnswer26(str);
			break;
		case 27:
			setAnswer27(str);
			break;
		case 28:
			setAnswer28(str);
			break;
		case 29:
			setAnswer29(str);
			break;
		case 30:
			setAnswer30(str);
			break;
		case 31:
			setAnswer31(str);
			break;
		case 32:
			setAnswer32(str);
			break;
		case 33:
			setAnswer33(str);
			break;
		case 34:
			setAnswer34(str);
			break;
		case 35:
			setAnswer35(str);
			break;
		case 36:
			setAnswer36(str);
			break;
		case 37:
			setAnswer37(str);
			break;
		case 38:
			setAnswer38(str);
			break;
		case 39:
			setAnswer39(str);
			break;
		case 40:
			setAnswer40(str);
			break;

		default:
			temp = null;
			break;
		}
	
		
	}
	
	public String getAnswerj(int index) {
		String temp = "";
		switch (index) {
		case 0:
			temp = getAnswer0();
			break;
		case 1:
			temp = getAnswer1();
			break;
		case 2:
			temp = getAnswer2();
			break;
		case 3:
			temp = getAnswer3();
			break;
		case 4:
			temp = getAnswer4();
			break;
		case 5:
			temp = getAnswer5();
			break;
		case 6:
			temp = getAnswer6();
			break;
		case 7:
			temp = getAnswer7();
			break;
		case 8:
			temp = getAnswer8();
			break;
		case 9:
			temp = getAnswer9();
			break;
		case 10:
			temp = getAnswer10();
			break;
		case 11:
			temp = getAnswer11();
			break;
		case 12:
			temp = getAnswer12();
			break;
		case 13:
			temp = getAnswer13();
			break;
		case 14:
			temp = getAnswer14();
			break;
		case 15:
			temp = getAnswer15();
			break;
		case 16:
			temp = getAnswer16();
			break;
		case 17:
			temp = getAnswer17();
			break;
		case 18:
			temp = getAnswer18();
			break;
		case 19:
			temp = getAnswer19();
			break;
		case 20:
			temp = getAnswer20();
			break;
		case 21:
			temp = getAnswer21();
			break;
		case 22:
			temp = getAnswer22();
			break;
		case 23:
			temp = getAnswer23();
			break;
		case 24:
			temp = getAnswer24();
			break;
		case 25:
			temp = getAnswer25();
			break;
		case 26:
			temp = getAnswer26();
			break;
		case 27:
			temp = getAnswer27();
			break;
		case 28:
			temp = getAnswer28();
			break;
		case 29:
			temp = getAnswer29();
			break;
		case 30:
			temp = getAnswer30();
			break;
		case 31:
			temp = getAnswer31();
			break;
		case 32:
			temp = getAnswer32();
			break;
		case 33:
			temp = getAnswer33();
			break;
		case 34:
			temp = getAnswer34();
			break;
		case 35:
			temp = getAnswer35();
			break;
		case 36:
			temp = getAnswer36();
			break;
		case 37:
			temp = getAnswer37();
			break;
		case 38:
			temp = getAnswer38();
			break;
		case 39:
			temp = getAnswer39();
			break;
		case 40:
			temp = getAnswer40();
			break;

		default:
			temp = null;
			break;
		}
		return temp;
	}
	/**
	 * @return the answer0
	 */
	public String getAnswer0() {
		return answer0;
	}
	/**
	 * @param answer0 the answer0 to set
	 */
	public void setAnswer0(String answer0) {
		this.answer0 = answer0;
	}
	
	

}
